...
render = () => {
  ...
  for (i = 0; i < enemies.length; i++) {
    if (enemy.x + enemy.width < 0) {
      ...
      redCoin.x = Math.floor(
        Math.random() * (screenW * 1.5 + bombDist - (screenW + bombDist)) +
          (screenW + bombDist)
      );
...
sonic.y -= 200
  ...
  */
  ...
  for (i = 0; i < myCoins.length; i++) {
    ...
    if (myCoins[i].x + myCoins[i].width < sonic.x) {
      ...
      redCoin.x = Math.floor(
        Math.random() * (screenW * 1.5 + bombDist - (screenW / 2 + bombDist)) +
          (screenW / 2 + bombDist)
      );
  ...
  */
  ...
  if (
    (coinScore >= 20 && coinScore < 25) ||
    (coinScore >= 65 && coinScore < 75) ||
    (coinScore >= 40 && coinScore < 50) ||
    (coinScore >= 85 && coinScore < 100)
  ) {
  ...
  if (
    sonic.x >= bluePortal.x &&
    sonic.x <= bluePortal.x + bluePortal.width &&
    sonic.y + sonic.height >= bluePortal.y
  ) {
    hitBlue = true;
    sonic.y = 1050;
